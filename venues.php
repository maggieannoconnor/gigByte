...
<body>
  ...
  <form name="mainForm" action="venues.php" method="post"> 
    ...
    <div class="row mb-3 mx-3">
      <input type="submit" value="Add Venue" name="actionBtn" 
              class="btn btn-primary" title="Insert a venue into venue" required />   
    </div>  
    <div class="row mb-3 mx-3">
      <input type="submit" value="Confirm Update" name="confirmUpdateBtn" 
              class="btn btn-secondary" title="Update a venues's information" required />   
    </div>  
  </form> 
  <hr/>
  <h3>Venues</h3>
  ...
  <?php foreach ($list_of_venues as $venue): ?> 
    <tr>  <!-- each row --> 
      <td><?php echo $venue['name']; ?></td> <!-- each column --> 
